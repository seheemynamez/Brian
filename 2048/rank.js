...
(function () {
  ...
  const ensureShareModalEl = () => {
    ...
    $('share-modal-share').addEventListener('click', async () => {
      ...
      // 주의 1: text + url 두 필드 분리해서 넘기면 카카오톡 같은 일부 앱이
      ...
      // 주의 2: title 도 카카오톡이 text 앞에 separator 없이 concat (예:
      // "2048 도전!HELLO 님 ..."). title 제거하면 메시지 본문 깔끔. 다른
      // 메신저는 title 을 share sheet 헤더로만 쓰니 영향 미미.
