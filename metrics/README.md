...
- `YYYY-MM-DD.json` — **KST 캘린더 day** 별 5분 collect snapshot 시계열 (KST 00:00 ~ 익일 00:00). snapshot 내부 `ts` 는 UTC ISO. **infra-only**: Render CPU/Mem/BW + Render events (deploy/server_failed) + Aiven metrics. server-domain 카운터 (worker_timeout / bot_retry / etc.) 는 **valkey 가 SoT 이며 snapshot 에 포함되지 않음** — daily-summary 가 `/api/daily-stats` endpoint 로 가져감.
- `state.json` — alert cooldown 상태 (같은 알림 종류 2시간 1회 제한). `last_alert` ts 는 UTC ISO.

> **이전 파일 `daily-stats.json` 제거** (valkey-first monitoring 전환). 일별 aggregate (pvp_games / bot_games / active_users / 등) 는 모두 server `/api/daily-stats?date=YYYY-MM-DD` 로 조회. 30일 보존이던 파일이 90일 보존 valkey Hash 로 대체. 7d trend 표는 daily-summary 가 7개 date 에 대해 endpoint 를 hit. backfill 은 `scripts/backfill_daily_stats_to_valkey.py` 로 옛 파일 데이터 옮길 수 있음 (one-time).
  ...
  "services": {
    "omok": {
      "render": {
        ...
        /* worker_timeout / bot_retry / bot_skip 등 server-domain 카운터는
           valkey 가 SoT — snapshot 에 포함되지 않음. /api/daily-stats endpoint 참고. */
      },
...
- snapshot 은 **infra-only** — server-domain 카운터 (worker_timeout / bot_retry / heartbeat 등) 는 valkey 가 SoT 이며 collect 가 더 이상 logs 에서 가져오지 않음. 단 collect 의 **burst alert** (worker_timeout / RETRY / SKIP burst Issue) 는 정확한 15분 window 측정이 필요해 그 부분만 log fetch 유지.
