...
- `daily-stats.json` — daily-summary 가 사용하는 일별 통계 누적 (30일치 보존). key = KST date. 필드: `pvp_games`, `bot_games`, `total_bot_moves`, `render_cpu_max_m`, `aiven_mem_max_pct`, `active_users` (24h game_over 의 unique 사람 닉네임 수), `total_human_users` (그 날 발행 시 server `/api/stats` 응답), `tiers` (발행 시점 사람 user 의 티어별 인원 — `{Iron, Bronze, Silver, Gold, Platinum, Diamond, Master}`).
  - **`pvp_games` / `bot_games` / `total_bot_moves` 소스 우선순위** (PR — A): server `/api/daily-stats?date=YYYY-MM-DD` (valkey HINCRBY 누적, authoritative) → 응답 None/0 시 Render 로그 fetch 폴백. 발행 로그에 `[daily-stats source] pvp=srv(N) bot=log(N) moves=...` 로 필드별 소스 명시. 처음 배포 직후엔 server 카운터가 비어 있어 자연스럽게 로그 폴백, 누적되면서 src 가 `srv` 로 전환.
